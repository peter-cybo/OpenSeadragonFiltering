...
(function () {
    ...
    const $ = window.OpenSeadragon || require('openseadragon');
    if (!$) {
    ...
    $.Filters = {
        ...
        RGBFUNCTION: function (red, green, blue, strength) {
            ...
            return function (context, callback) {
                let imgData = context.getImageData(0, 0, context.canvas.width, context.canvas.height, { willReadFrequently: true });
                let pixels = imgData.data;

                for (let i = 0; i < pixels.length; i += 4) {
                    pixels[i] = Math.min(255, pixels[i] + red * strength);
                    pixels[i + 1] = Math.min(255, pixels[i + 1] + green * strength);
                    pixels[i + 2] = Math.min(255, pixels[i + 2] + blue * strength);
                }
